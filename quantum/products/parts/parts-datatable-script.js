  ...
  if (filterStatus === SEARCH_FILTER || filterStatus === SEARCH_MODEL) {
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        let { total, item_qty } = d;
        $(".total_rows > span").html(item_qty);
...
function fetchData() {
  ...
  if (filterStatus === SEARCH_IMEI) {
    ...
    $.ajax({
      type: "POST",
      url: "includes/fetch-parts-data.php",
      data: {
        stockType,
        searchItemCode,
      },
...
const drawTableRow = function (rowsData, isFilterData) {
  ...
  rowsData.forEach((val) => {
    let { item_code, product_title, item_qty, item_brand, item_color } = val;
...
function drawSummaryData() {
  ...
  selectedArray = _.groupBy(summaryDataset, (item) => {
    return `${
      ...
      (!!item.item_color ? item.item_color.split(" ").join("") : "") +
  ...
  selectedArray.forEach((item) => {
    let { category, product_title, total, item_qty, item_color } = item;
    let tr = `<tr>
            <td colspan="4"><b>
                ...
                <span style="text-transform:capitalize;" class="selected-model">${
