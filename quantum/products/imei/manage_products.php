  ...
  <div class="content">
    <div class="row">
      <div class="col-md-12">
        ...
        <?php
                $fetch_colors = mysqli_query($conn, "select distinct item_color
                from tbl_imei ORDER BY item_color ASC")
                or die('Error: '.mysqli_error($conn));
                $colorOptions = [];
                while($color = mysqli_fetch_assoc($fetch_colors)){
                  $value = trim((string)$color['item_color']);
                  if(strlen($value) > 0){
                    $colorOptions[] = $value;
                  }
                }

                $fetch_oem_colors = mysqli_query($conn, "select distinct oem_color
                from tbl_imei ORDER BY oem_color ASC")
                or die('Error: '.mysqli_error($conn));
                $oemColorOptions = [];
                while($oemColor = mysqli_fetch_assoc($fetch_oem_colors)){
                  $value = trim((string)$oemColor['oem_color']);
                  if(strlen($value) > 0){
                    $oemColorOptions[] = $value;
                  }
                }
        ?>
        <label for="" style="width:10%; position:relative;">
        ...
        <label for="" style="width:10%; position:relative;">
          <input type="text" placeholder="OEM Color" class="form-control select-oem-color" autocomplete="off"
            data-oem-color-options='<?php echo htmlspecialchars(json_encode($oemColorOptions), ENT_QUOTES, "UTF-8"); ?>'>
          <div class="oem-color-suggestion-panel hide"></div>
        </label>
        ...
        <label for="" style="width:10%;">
          <select placeholder="GB" class="form-control select-gb">
            ...
            <?php
                    $fetch_gb_values = mysqli_query($conn, "select id, gb_value
                    from tbl_gb order by gb_value+0")
                    or die('Error: '.mysqli_error($conn));
        ...
        <label for="" style="width:10%;">
          <select placeholder="Grade" class="form-control select-grade">
            <option value="">Grade</option>
            <?php 
                    $fetch_grades = mysqli_query($conn, "select grade_id, title
                    from tbl_grades")
                    or die('Error: '.mysqli_error($conn)); 
                    while($grade = mysqli_fetch_assoc($fetch_grades)):
                    ?>
            <option value="<?php echo $grade['grade_id'];?>"><?php echo $grade['title'];?></option>
            <?php 
                endwhile; ?>
          </select>
        </label>
        ...
        <label for="" style="width:15%;">
          <select class="form-control select-qc-done">
            <option value="">QC Done?</option>
        ...
        <label for="" style="width:10%;">
          <select class="form-control select-qc-status">
        ...
        <label for="" style="width:12%;">
          <input type="text" placeholder="From:" name="from_date" class="form-control from_date" id="from_date"
            required>
        </label>


        <label for="" style="width:12%;">
          <input type="text" name="to_date" placeholder="To:" class="form-control to_date" id="to_date" required>
        </label>

        <button class="apply-filters btn btn-success" style="width:12%;">
          Apply
        </button>

        <div style="width:45%;margin-right:10px;" class="filter-group pull-left">
          <input type="text" class="pull-left form-control search-datatable-input" placeholder="Search IMEI"
        ...
        <div style="width:45%;" class="filter-group pull-left">
          <input type="text" class="pull-left form-control search-model-input" placeholder="Search Model"
            style="width:80%;padding:22px 20px !important; font-size:22px;">
          <button style="width:20%;" class="pull-right btn btn-success btn-lg search-model">Search</button>
        </div>
          ...
          <div class="box box-success">
            ...
            <div class="box-body custom_datatable_wrapper">
              ...
              <div class="custom_datatable">
                ...
                <button type="button" style="margin-right:10px;" class="btn btn-default view-summary-btn pull-right"
                ...
                <table class="export-table table hide" id="export-table">
                  <thead>
                    <tr>
                      ...
                      <th>Brand</th>
                      <th>Supplier</th>
                      <th>Location</th><!-- Moved to after Supplier -->
                      <th>Color</th>
                      <th>OEM Color</th>
                      <th>GB</th>
                      <th>Grade</th>
              ...
              <select class="hide form-control brands-field">
                <?php 	
                      ...
                      ?>
                <option value="<?php echo $get_category2['category_id'];?>"><?php echo $get_category2['title']; ?>
                </option>
                <?php endwhile;?>
              </select>
              ...
              <select class="hide form-control supplier-field">
                <?php 	
                      ...
                      ?>
                <option value="<?php echo $get_supplier2['supplier_id'];?>"><?php echo $get_supplier2['name']; ?>
                </option>
                <?php endwhile;?>
              </select>
...
<script src="imei-datatable-script.js"></script>
</body>
