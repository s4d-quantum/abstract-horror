...
<?php
    ...
    if(isset($_POST['searchIMEI'])){
        ...
        $isQcPassed = isset($_POST['isQcPassed']) && $_POST['isQcPassed'] != '' ? $isQcPassed = $_POST['isQcPassed']:NULL;
        $stockType = isset($_POST['stockType']) ? $_POST['stockType'] : null; // FIXED HERE
    
    // BASIC QUERY
    $query = "
        select 
        count(im.item_imei) as total,
        ";

        // IF CUSTOMER
        if(!is_null($customer) || $stockType === 'sold'){
            $query .= "ord.customer_id," ;
        }
    ...
    // Return as plain number, not JSON encoded, since it's just a single value
    echo $total_rows_count;
?>
