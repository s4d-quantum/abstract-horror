...
<?php
    ...
    $query = "
        ...
        // IF STOCK TYPE
        $query .= $stockType === 'instock' ? " i.status = 1 AND pr.purchase_return = 0 ":" (i.status = 0 OR i.status = 1) ";
        ...
        // IF GRADE 
        $query .= !is_null($grade) ? " AND i.item_grade ='".$grade."'" : "";
    ...
    if(isset($_POST['searchIMEI'])){
        ...
        $fetch_items = mysqli_query($conn,
        $query." ORDER BY i.id DESC LIMIT 10 OFFSET ".$pageId)
        or die('Error: '.mysqli_error($conn)); 
    }
