...
let tableState = {
  ...
  oemColor: urlParams.get("OEMCOLOR") || null,
...
// Escape HTML for safe injection in suggestion panels
...
function initializeSuggestionInput({
  ...
  const input = D[QS](inputSelector);
  if (!input) return;

  const suggestionPanel = input.parentElement.querySelector(panelSelector);
  if (!suggestionPanel) return;

  let options = [];
  try {
    options = JSON.parse(input.dataset[datasetKey] || "[]");
  } catch (error) {
    options = [];
  }
  ...
  function showSuggestions(results, queryProvided) {
    if (!results.length) {
      if (queryProvided) {
        suggestionPanel.innerHTML = `<div class="${itemClass}" style="padding:8px 12px; color:#777; cursor:default;">No matches found</div>`;
    ...
    const itemsMarkup = results
      .map(function (value) {
        const escaped = escapeHtml(value);
        return `<div class="${itemClass}" data-value="${escaped}" style="padding:8px 12px; cursor:pointer;">${escaped}</div>`;
      })
      .join("");
  ...
  function filterOptions(query) {
    ...
    const filtered = options.filter(function (value) {
  ...
  suggestionPanel.addEventListener("mousedown", function (event) {
    ...
    if (!target.classList.contains(itemClass)) return;
    const selectedValue = target.getAttribute("data-value") || "";
    input.value = selectedValue;
    hideSuggestions();
  });

  if (tableState[stateKey]) {
    input.value = tableState[stateKey];
  }
...
initializeSuggestionInput({
  inputSelector: ".select-color",
  panelSelector: ".color-suggestion-panel",
  datasetKey: "colorOptions",
  stateKey: "color",
  itemClass: "color-suggestion-item",
});

initializeSuggestionInput({
  inputSelector: ".select-oem-color",
  panelSelector: ".oem-color-suggestion-panel",
  datasetKey: "oemColorOptions",
  stateKey: "oemColor",
  itemClass: "oem-color-suggestion-item",
});
...
function fetchExportData(callback) {
  ...
  $.ajax({
    ...
    data: {
      ...
      isQcPassed,
      stockType,
      // pageId: currentPage, // REMOVED - we need ALL results for export/summary, not just current page
    },
    success: function (data) {
      data = JSON.parse(data);
      let rowsData = data.data;
      ...
      OEM_Color: row.oem_color,
        GB: row.item_gb,
        Po_Box: row.po_box,
        Grade: findGrade(row.item_grade),
        Status: row.reserved_order_id
          ? `Reserved (SO-${row.reserved_order_id})`
          : row.status == 1 ? "Available" : "Not Available",
      ...
      // Populate summaryDataset from ALL export data (not just current page)
      summaryDataset = rowsData.map((row) => {
        let category = "";
        if (row.item_brand && row.item_brand.length > 0) {
          category = findBrand(row.item_brand);
        }

        let supplier = "";
        if (row.supplier_id && row.supplier_id.length > 0) {
          supplier = findSupplier(row.supplier_id);
        }

        let grade = findGrade(row.item_grade);

        return {
          purchase_id: row.purchase_id,
          item_imei: row.item_imei,
          category: category,
          supplier: supplier,
...
function drawSummaryData() {
  ...
  selectedArray = _.groupBy(summaryDataset, (item) => {
    return `${
      ...
      (item.oem_color ? item.oem_color.split(" ").join("") : "") +
  ...
  selectedArray.forEach((item) => {
    ...
    let tr = `<tr>
            <td colspan="4"><b>
                ...
                ${
                  oem_color
                    ? `<span style="text-transform:capitalize;" class="selected-oem-color">OEM ${oem_color}</span>`
                    : ""
                }
                <span style="text-transform:capitalize;" class="selected-gb">${
                  item_gb.length > 0 ? (parseInt(item_gb, 10) >= 1024 ? (parseInt(item_gb, 10) / 1024) + ' TB' : item_gb + ' GB') : ''
                }</span>
                ${
                  grade !== "-"
...
function processData(data) {
  ...
  data.forEach(item => {
    ...
    exportRows += `<tr>
      ...
      <td><input type="hidden" name="item_oem_color[]" value="${item.oem_color || ''}">${item.oem_color || ''}</td>
...
function filtersCalled() {
  ...
  // OEM color filter
  let oemColor = document.getElementsByClassName("select-oem-color")[0].value;
  if (oemColor.length <= 0) {
    oemColor = null;
  }
  tableState.oemColor = oemColor;
  url = updateUrl(url, oemColor, "OEMCOLOR", tableState.oemColor);
