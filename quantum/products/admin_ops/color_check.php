...
$colorCodeStyleMap = [
  'AK' => 'background-color:#2b2b2b;color:#ffffff;',
  'DB' => 'background-color:#1f4e79;color:#ffffff;',
  'DR' => 'background-color:#8b2c2c;color:#ffffff;',
  'ID' => 'background-color:#d4af7f;color:#3f2f17;',
  'LB' => 'background-color:#b9dcff;color:#1f4e79;',
  'LG' => 'background-color:#cfecc7;color:#2f6b2f;',
  'LI' => 'background-color:#d8c6f0;color:#5f4685;',
  'LM' => 'background-color:#d7f06b;color:#4d5f00;',
  'LV' => 'background-color:#c9b2ff;color:#4d2d7a;',
  'ZA' => 'background-color:#8f8f95;color:#ffffff;',
  'ZD' => 'background-color:#d4af37;color:#4b3a00;',
  'ZE' => 'background-color:#f4ecd8;color:#7a6840;',
  'ZG' => 'background-color:#b7d7a8;color:#2f6b2f;',
  'ZI' => 'background-color:#f6c1d0;color:#9a3d63;',
  'ZK' => 'background-color:#1f1f1f;color:#ffffff;',
  'ZN' => 'background-color:#8b6a4f;color:#ffffff;',
  'ZR' => 'background-color:#d9534f;color:#ffffff;',
  'ZS' => 'background-color:#d9dde2;color:#4d5966;',
  'ZT' => 'background-color:#8a817c;color:#ffffff;',
  'ZV' => 'background-color:#b39ddb;color:#4a2f7a;',
  'ZW' => 'background-color:#f8f9fa;color:#555555;',
];
$statistics = [
  'total' => 0,
  'internal_api' => 0,
...
function renderModelNumberWithColorCode(?string $modelNumber, array $colorCodeMap, array $colorCodeStyleMap): string
{
  if (!is_string($modelNumber)) {
    return '';
  }
  ...
  if (strlen($modelNumber) < 10) {
    return htmlspecialchars($modelNumber, ENT_QUOTES, 'UTF-8');
  }

  $colorCode = strtoupper(substr($modelNumber, 8, 2));
  $colorName = $colorCodeMap[$colorCode] ?? 'Unknown color code';
  $colorStyle = $colorCodeStyleMap[$colorCode] ?? 'background-color:#fff3cd;color:#8a5a00;';

  $prefix = htmlspecialchars(substr($modelNumber, 0, 8), ENT_QUOTES, 'UTF-8');
  $suffix = htmlspecialchars(substr($modelNumber, 10), ENT_QUOTES, 'UTF-8');
  $colorCodeHtml = htmlspecialchars($colorCode, ENT_QUOTES, 'UTF-8');
  $tooltip = htmlspecialchars($colorName, ENT_QUOTES, 'UTF-8');
  $style = htmlspecialchars($colorStyle, ENT_QUOTES, 'UTF-8');

  return $prefix
    . '<span class="model-color-code" style="' . $style . '" data-toggle="tooltip" data-placement="top" title="' . $tooltip . '">'
    . $colorCodeHtml
    . '</span>'
    . $suffix;
}
              ...
              <?php if (!empty($results)): ?>
                <div class="box">
                  <div class="box-header">
                    ...
                    <a href="javascript:void(0);" class="color-code-link open-color-codes-modal">Color Codes</a>
                  </div>
                      ...
                      <tbody>
                        <?php foreach ($results as $row): ?>
                          <tr>
                            ...
                            <td><?php echo renderModelNumberWithColorCode($row['details']['model_number'] ?? '', $colorCodeMap, $colorCodeStyleMap); ?></td>
...
<?php if (!empty($results)): ?>
  <div class="modal fade" id="colorCodesModal" tabindex="-1" role="dialog" aria-labelledby="colorCodesModalLabel">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" id="colorCodesModalLabel">Color Codes</h4>
        </div>
        <div class="modal-body">
          <div class="table-responsive">
            <table class="table table-bordered table-striped">
              ...
              <tbody>
                <?php foreach ($colorCodeMap as $code => $colorName): ?>
                  <tr>
                    <td><?php echo htmlspecialchars($code); ?></td>
                    <td><?php echo htmlspecialchars($colorName); ?></td>
                  </tr>
                <?php endforeach; ?>
              </tbody>
            </table>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (window.jQuery) {
    const $colorCodesModal = window.jQuery('#colorCodesModal');
    const $colorCodesTrigger = window.jQuery('.open-color-codes-modal');

    if ($colorCodesModal.length) {
      $colorCodesModal.appendTo('body');
    }

    if ($colorCodesModal.length && $colorCodesTrigger.length) {
      $colorCodesTrigger.on('click', function(e) {
        e.preventDefault();
        e.stopPropagation();
        $colorCodesModal.modal('show');
      });
    }
  }
});
