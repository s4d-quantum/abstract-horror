<?php include '../../../db_config.php';  ?>
  ...
  $fetch_incomplete = mysqli_query($conn, "select distinct purchase_id from tbl_purchases where 
  ...
  // first find if any the pid is available in tbl_qc_imei_products, 
  $pid_in_qc = mysqli_query($conn, "select count(purchase_id) as total from tbl_qc_imei_products 
  ...
  if($is_pid_in_qc['total'] == 0){
    $fetch_all_items = mysqli_query($conn, "select count(item_imei) as total from tbl_purchases  
    where purchase_id=".$pid['purchase_id'])
    or die('Error: '.mysqli_error($conn)); 
  ...
  // 2 - if yes then find how many are pending 
    ...
    $fetch_pending_rows = mysqli_query($conn, "select count(item_code) as total from tbl_qc_imei_products 
    where  
