...
<?php 
  ...
  $purchase_details_query = mysqli_query($conn,"select * from tbl_parts_purchases 
  ...
  if(isset($_POST['submit_purchase'])){
    ...
    // UPDATE ITEM QTY
    $fetch_prev_items = mysqli_query($conn,"select * from tbl_parts_purchases 
    where purchase_id = '".$new_pur_id."'")
    or die('Error:: ' . mysqli_error($conn));
    ...
    // DEL PREV ENTRIES
    $fetch_prev_items = mysqli_query($conn,"delete from tbl_parts_purchases 
    where purchase_id = '".$new_pur_id."'")
    or die('Error:: ' . mysqli_error($conn));
              ...
              <table class="table" id="purchase_items">
                  ...
                  <tbody>
                    <?php 
                      ...
                      pd.id
                      from 
                      tbl_parts_purchases as pr inner join tbl_parts_products as pd 
                      ...
                      <td>
                        <select name="item_code[]"  class="code-field form-control">
                          ...
                          <?php 
                            ...
                            where item_brand = '".$row['item_brand']."' AND char_length(item_tac) > 0");
