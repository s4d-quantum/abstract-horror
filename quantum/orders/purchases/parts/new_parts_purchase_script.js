...
function validateDetails() {
  ...
  let allInputValues = [...document.getElementsByClassName("item_code")].filter(
    (input) => {
...
$(document).on("click", ".submit-form", function (e) {
  ...
  let checkcode = Array.prototype.filter.call(
    code,
    (item) => item.value.length <= 0
  );
  // check itemqtys
  let itemqty = document.querySelectorAll(".item_qty");
  let checkitemqty = Array.prototype.filter.call(
    itemqty,
    (item) => parseInt(item.value) <= 0 || item.value.length <= 0
  );
...
function fetchTacs() {
  $.ajax({
    type: "POST",
    url: "includes/fetch_tacs_for_goodsin.php",
    data: {},
    success: function (data) {
      globals.tacs = JSON.parse(data).map((tac) => ({
        tacTitle: tac.item_details.trim(),
        tacId: tac.item_tac.trim(),
      }));
    },
    error: function (data) {},
  });
...
function selectBrandsItems() {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $(that).parent().siblings().find(".code-field").html("");

      let items = JSON.parse(data);
      console.log(data);

      // reset tacs
      $(that).parent().siblings().find(".item-tacs").empty();
      // filter associated tacs
      items.map((item) => {
        let tempTac = !!item.item_tac ? item.item_tac.trim() : "";
        // each item may have multiple tacs

        tempTac.split(",").map((tac) => {
          globals.associatedTacs.push({
            tac: tac.trim(),
            itemId: item.id,
          });
        });
          ...
          .forEach((item) => {
            ...
            if (itemCode.length) {
              $(that)
                ...
                .find(".code-field")
                .append(
                  `<option value="${itemId}">${`${itemCode} ${
                    item.item_color && item.item_color.length
                      ? " - " + item.item_color
                      : ""
                  }`}</option>`
                );
...
// Update tacs on item code change
$(document).on("change", ".code-field", function (e) {
  // Empty tacs
  $(this).parent().siblings().find(".item-tacs").empty();
  // populate tacs
  globals.associatedTacs
    .filter((item) => item.itemId === e.target.value)
    .map((item) => {
      let title = globals.tacs.find((tac) => tac.tacId === item.tac);
      if (!!title) {
        $(this)
          .parent()
          .siblings()
          .find(".item-tacs")
          .append(
            `<span style="margin-right:2px;" class="label label-default label-secondary text-xs text-normal">${title.tacTitle}</span>`
          );
      }
    });
});
