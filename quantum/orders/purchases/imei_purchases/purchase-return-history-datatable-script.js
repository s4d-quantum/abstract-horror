...
const drawTableRow = function (rowsData) {
  ...
  if (!rowsData.length) {
    ...
    rowsData.forEach((val) => {
      let tr = document.createElement("tr");
      let { return_id, date, purchase_id, supplier_id, total, is_delivered } =
        val;
      ...
      tr.innerHTML = `<tr>
            <td>
              ${return_id}
              <input type="text" value=${return_id} class="return_id hide"/>
            </td>
            <td>${date}</td>
            <td>${supplier}</td>
            <td>
          ...
          <td>
          <button
            class="btn btn-warning edit_return"
            data-return-id="${return_id}"
            data-toggle="modal"
            data-target="#modal-edit">
            <i class="fa fa-pencil"></i>
          </button>

          <a href="purchase_return_details.php?ret_id=${return_id}"
...
$(document).on("click", ".edit_return", function (e) {
  e.preventDefault();
  // set and show purchase return id in modal title
  let returnId = this.dataset.returnId;
  D[QS](".modal-return-id").innerHTML = `IPR-${returnId}`;
  D[QS](".modal-return-id-input").value = returnId;

  $.ajax({
    type: "POST",
    url: "includes/fetch_tracking_details.php",
    data: {
      return_id: returnId,
    },
    success: function (d) {
      let data = JSON.parse(d);
      D[QS](".modal-delivery-company").value = data.delivery_company;
      D[QS](".modal-tracking-no").value = data.tracking_no;
    },
    error: function (data) {},
  });
});
