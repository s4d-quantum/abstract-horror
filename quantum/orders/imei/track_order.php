...
if ($delivery_company_key === "dpd") {
    ...
    if (!empty($dpd_creds['error'])) {
        echo '<div class="alert alert-danger">DPD tracking error: ' . htmlspecialchars($dpd_creds['error']) . '</div>';
    } else {
...
XML;
        ...
        if ($result === false || ($http_status !== null && $http_status >= 400)) {
            $status_msg = $http_status ? "HTTP {$http_status}" : 'unknown error';
            error_log("DPD tracking request failed: {$status_msg}");
            echo '<div class="alert alert-danger">DPD tracking request failed.</div>';
        } else {
            ...
            if ($xml === false) {
                ...
                libxml_clear_errors();
                error_log('DPD tracking XML parse error: ' . implode('; ', $errors));
            } elseif (isset($xml->trackingdetails->trackingdetail->trackingevents)) {
