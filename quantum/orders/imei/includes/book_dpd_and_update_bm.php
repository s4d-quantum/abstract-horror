...
add_debug("Using DPD printer endpoint: {$dpd_printer_endpoint}");

$command = sprintf(
    "php $bookdpd_path --fn %s --ln %s --ph %s --st %s --city %s --pc %s --em %s --price %s --ref %s --locality %s --cutoff %s --printer %s",
    ...
    escapeshellarg($dpd_printer_endpoint)
);
...
if (!$dpd_only) {
    ...
    $command = sprintf(
        "php $update_bm_path -oid %s -imei %s -tr %s -dn %s",
        escapeshellarg($po_ref),
