...
if(isset($_POST['item_imei'])){
    ...
    // CRITICAL FIX: Must check status=1 even for items in this sales order to prevent
    // items that have already been processed to goods out from being scanned again
    $availabilityClause = "
        (
            (im.in_sales_order = {$sales_order_id} AND im.status = 1)
            OR (
                im.status = 1
                AND NOT EXISTS (
                    SELECT 1
