...
<?php 
  ...
  // fetch sales order id to check if this is a Backmarket order
  $fetch_sales_id = mysqli_query($conn,"select order_id as sales_order_id from tbl_imei_sales_orders where
  goodsout_order_id=".$order_id)
  or die('Error:: ' . mysqli_error($conn));
  $sales_id = mysqli_fetch_assoc($fetch_sales_id);
 
  // Fetch PO ref for BackMarket if applicable
  $bm_po_ref = '';
  if ($sales_id) {
      $fetch_po_query = mysqli_query($conn, "SELECT po_ref FROM tbl_imei_sales_orders WHERE order_id = " . $sales_id['sales_order_id'] . " LIMIT 1");
      if ($fetch_po_query && $po_row = mysqli_fetch_assoc($fetch_po_query)) {
          $bm_po_ref = $po_row['po_ref'];
      }
  }
 
  // check if this is a Backmarket order (customer_id "CST-78")
  $is_backmarket_order = false;
  if ($sales_id) {
    $fetch_sales_customer = mysqli_query($conn,"select customer_id from tbl_imei_sales_orders where
    order_id=".$sales_id['sales_order_id'])
    or die('Error:: ' . mysqli_error($conn));
    $sales_customer = mysqli_fetch_assoc($fetch_sales_customer);
    $is_backmarket_order = ($sales_customer && $sales_customer['customer_id'] === 'CST-78');
  }
...
// Redirect to the same page without the email parameter to prevent resending
header("Location: order_details.php?ord_id=" . $_GET['ord_id']);
exit();
}
...
<!-- Page loader -->
<div class="page-loader no-print" style="display:none;">
              ...
              <div class="col-xs-12">
                <div class="box dispatch-note">
                  <div class="box-header">
                    ...
                    <?php if ($is_backmarket_order): ?>
                    <button class="btn btn-warning pull-right no-print update-bm" name="update-bm"
                      style="margin-right:10px;" data-backmarket="true">
                      <i class="fa fa-truck"></i> Update BM
                    </button>
                    <?php endif; ?>
...
<!-- DPD Booking Progress Modal -->
<div class="modal fade" id="dpd-progress-modal">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Book DPD and Update BM Progress</h4>
      </div>
      <div class="modal-body">
        <div id="progress-steps">
          <p id="step1"><i class="fa fa-spinner fa-spin"></i> Getting BM order ID...</p>
          <p id="step2"><i class="fa fa-circle-o"></i> Fetching BM order info...</p>
          <p id="step3"><i class="fa fa-circle-o"></i> Logging into DPD...</p>
          <p id="step4"><i class="fa fa-circle-o"></i> Booking DPD shipment...</p>
        </div>
        <div id="progress-details" style="margin-top: 20px; font-family: monospace; white-space: pre-wrap;"></div>
      </div>
      <div class="modal-footer">
...
// Fetch latest tracking status for supported carriers on this page.
const statusCarrier = <?php echo json_encode(strtoupper(trim($get_purchase_details['delivery_company'] ?? ''))); ?>;
const statusOrderId = <?php echo (int)$order_id; ?>;
const trackingText = document.getElementById('dpd-status').innerText.trim();
const supportsInlineTrackingStatus = statusCarrier === 'DPD' || statusCarrier === 'DHL';

if (supportsInlineTrackingStatus && !!trackingText && trackingText.length > 5) {
  $.ajax({
    type: "POST",
    ...
    success: function(response) {
      if (response && !response.error && response.status) {
        $('#dpd-result').text(` - ${response.status}`);
      } else {
        $('#dpd-result').text("");
      }
    },
...
// Handle Update BM button click
$(document).on("click", ".update-bm", function(e) {
  e.preventDefault();

  // Check if this is a Backmarket order
  ...
  const salesOrderId = <?php echo isset($sales_id['sales_order_id']) ? (int)$sales_id['sales_order_id'] : 0; ?>;

  if (!orderId && !salesOrderId) {
    alert("Missing order reference. Please reload the page and try again.");
    return;
  }

  // Confirm with user before proceeding
  if (!confirm("Update BackMarket with the current shipment details?")) {
    return;
  }

  $(".page-loader").show();
  $(".update-bm").prop("disabled", true);

  // Call the PHP script to trigger update_bm.php with stored data
  $.ajax({
    type: "POST",
    url: "includes/run_update_bm.php",
    data: {
      order_id: orderId,
    ...
    success: function(response) {
      ...
      if (result.success) {
        ...
        alert((result.message || "BackMarket updated successfully.") + detailMessage);
      } else {
        const errorDetails = result.details ? "\n\nDetails:\n" + result.details : "";
        alert("Error: " + (result.message || "Unable to update BackMarket.") + errorDetails);
      }
    },
    error: function(xhr, status, error) {
      console.error("AJAX Error:", error);
      console.error("Response:", xhr.responseText);
      let message = "An error occurred while updating BackMarket.";
      if (xhr.responseText) {
        message += "\n\nDetails:\n" + xhr.responseText;
      }
      alert(message);
...
<script>
// Auto-print dispatch note after new order redirect with toast feedback
(function() {
  function ensureToastStyles() {
    if (document.getElementById('print-toast-style')) return;
    var style = document.createElement('style');
    style.id = 'print-toast-style';
    style.textContent = '
      .print-toast { position: fixed; top: 16px; right: 16px; z-index: 99999; }
      .print-toast .toast-msg { min-width: 240px; max-width: 360px; margin-top: 8px; padding: 10px 12px; border-radius: 4px; color: #fff; box-shadow: 0 2px 6px rgba(0,0,0,0.2); font-size: 13px; }
      .print-toast .info { background: #3c8dbc; }
      .print-toast .success { background: #00a65a; }
      .print-toast .warning { background: #f39c12; }
      .print-toast .error { background: #dd4b39; }
    ';
    document.head.appendChild(style);
  }
  function showToast(message, type) {
    ensureToastStyles();
    var cont = document.getElementById('print-toast');
    if (!cont) {
      cont = document.createElement('div');
      cont.id = 'print-toast';
      cont.className = 'print-toast';
      document.body.appendChild(cont);
    }
    var div = document.createElement('div');
    div.className = 'toast-msg ' + (type || 'info');
    div.textContent = message;
    cont.appendChild(div);
    setTimeout(function(){
      if (div.parentNode) div.parentNode.removeChild(div);
      if (cont && !cont.firstChild && cont.parentNode) cont.parentNode.removeChild(cont);
    }, 6000);
  }
  ...
  try {
    ...
    if (params.get('autoprint') === '1') {
      showToast('Sending dispatch note to printer…', 'info');

      var url = 'includes/pdf_note.php?ord_id=<?= htmlspecialchars($order_id, ENT_QUOTES) ?>';
      fetch(url, { credentials: 'same-origin' })
        .then(function(resp) {
          var ct = (resp.headers.get('content-type') || '').toLowerCase();
          if (ct.indexOf('application/json') !== -1) {
            return resp.json().then(function(data){
              showToast('Dispatch note sent to printer.', 'success');
            }).catch(function(){
              showToast('Printer responded, but could not parse reply.', 'warning');
            });
          } else if (ct.indexOf('application/pdf') !== -1) {
            // Fallback from server: webhook failed; PDF streamed
            showToast('Printer not reached. PDF fallback generated.', 'warning');
            return null;
          } else if (resp.ok) {
            showToast('Printer response received.', 'success');
            return null;
          } else {
            showToast('Failed to send to printer (' + resp.status + ').', 'error');
            return null;
          }
        })
        .catch(function() {
          showToast('Failed to contact printer service.', 'error');
        });

      // Remove the flag to avoid re-triggering on refresh
