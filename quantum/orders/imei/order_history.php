...
if(isset($_GET['del'])){
  ...
  // SECURITY FIX: Check if order exists and validate permissions
  $check_order = mysqli_query($conn,"SELECT order_id, customer_id, user_id FROM tbl_orders WHERE order_id = '".$order_id."' LIMIT 1")
  ...
  // Check if user has permission (admin or order creator)
  if ($_SESSION['user_role'] !== 'admin' && $order_data['user_id'] != $user_id) {
    die('Error: Unauthorized - you do not have permission to delete this order');
  }

  // Check if order has already been confirmed/shipped
  $check_status = mysqli_query($conn,"SELECT unit_confirmed FROM tbl_orders WHERE order_id = '".$order_id."' AND unit_confirmed = 1 LIMIT 1");
  if (mysqli_num_rows($check_status) > 0) {
    die('Error: Cannot delete order that has already been confirmed/shipped');
  }
