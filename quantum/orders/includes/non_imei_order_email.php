...
$get_email_query = mysqli_query($conn,"select * from tbl_settings")
    ...
    $message = '<h2 style="padding:10px; text-align:center; background:#8ba2bf; color:#ffffff;">Non IMEI Goods Out Report</h2>';
    $message .= '<p style="margin:5px;"> Order ID: <b>'.$_POST['order_id'].'</b></p>';
    $message .= '<p style="margin:5px;">Date: <b>'.$_POST['date'].'</b></p>';
    $message .= '<p style="margin:5px;">Customer: <b>'.$_POST['customer'].'</b></p>';
    $message .= '<table cellpadding="10" style="margin:10px 0; width:100%;" border="1">';
    $message .= '<thead style="background:#cbd6e4"><th>Item Code</th><th>Brand</th><th>Qty</th><th>Details</th></thead>';
    ...
    $recipient = $get_email;
    $subject = "S4DLimited GoodsOut Report";
