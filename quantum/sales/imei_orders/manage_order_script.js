...
let selectedOrderId = "";
// Toggle state: include BackMarket orders (customer_id = 'CST-78')
let includeBM = false;
// Initialize from hidden input if present (used by manage_all_orders.php)
const __bmDefault = document.getElementById('include_bm_default');
if (__bmDefault && __bmDefault.value === '1') {
  includeBM = true;
}
...
// Toggle BM orders visibility from top-right button
const toggleBmBtn = document.getElementById("toggle-bm-orders");
function updateBmButtonState(btn) {
  if (!btn) return;
  btn.classList.toggle("btn-warning", includeBM);
  btn.innerHTML = `<i class="fa fa-exchange"></i> ${includeBM ? 'Showing All (incl. BM)' : 'Hiding BM Orders'}`;
}
if (toggleBmBtn) {
  updateBmButtonState(toggleBmBtn);
  toggleBmBtn.addEventListener("click", function () {
    includeBM = !includeBM;
    updateBmButtonState(this);
    // clear search on toggle for clarity
    const s = document.querySelector('.search-datatable');
    if (s) s.value = '';
...
// jQuery delegated fallback (in case of dynamic DOM or other scripts)
$(document).on('click', '#toggle-bm-orders', function() {
  includeBM = !includeBM;
  updateBmButtonState(this);
  const s = document.querySelector('.search-datatable');
  if (s) s.value = '';
