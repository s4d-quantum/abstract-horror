  ...
  <div class="content-wrapper">
    ...
    <div class="box box-success">
      <div class="box-body">
        ...
        <h3>Orders from: <b><?php echo $from_date; ?></b> to <b><?php echo $to_date; ?></b></h3>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            ...
            <?php $get_date_purchases = mysqli_query($conn,"select * from tbl_orders where date='".$date."'")
              or die('Error:: ' . mysqli_error($conn));
             ?>
            <?php while($row = mysqli_fetch_assoc($get_date_purchases)): ?>
            
            <?php $get_purchase_items_query = mysqli_query($conn,"select distinct im.item_imei,
            im.item_tac,tc.item_details,tc.item_gb,tc.item_brand,ord.date,ord.is_return,
            im.order_return from 
            tbl_orders as ord inner join tbl_imei as im on im.item_imei = ord.item_imei 
            inner join tbl_tac as tc on im.item_tac = tc.item_tac where 
            ord.item_imei = '".$row['item_imei']."'")
              or die('Error:: ' . mysqli_error($conn));
