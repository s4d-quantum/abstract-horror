...
$query="select distinct pr.item_imei, pr.purchase_id, pr.qc_required, pr.qc_completed from tbl_purchases pr where pr.purchase_id = '".$p_id."'";

$result = mysqli_query($conn,$query)
or die('Error:: ' . mysqli_error($conn));

$color_options = [];
$color_query = mysqli_query($conn,"select distinct item_color from tbl_imei where item_color IS NOT NULL AND item_color <> '' order by item_color asc")
or die('Error:: ' . mysqli_error($conn));
while($color_row = mysqli_fetch_assoc($color_query)){
  $color_options[] = $color_row['item_color'];
}
              ...
              <div class="col-xs-12">
                <div class="box">                
                  ...
                  <div class="box-body">
                    ...
                    <datalist id="qc-color-options">
                      <?php foreach ($color_options as $color_option): ?>
                        <option value="<?php echo htmlspecialchars($color_option, ENT_QUOTES, 'UTF-8'); ?>"></option>
                      <?php endforeach; ?>
                    </datalist>
                    <table id="example1" class="table table-bordered">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td class="qc-color-cell">
                            <div class="input-group input-group-sm qc-color-editor">
                              <input
                                type="text"
                                name="item_color[]"
                                class="form-control qc-color-input"
                                value="<?php echo htmlspecialchars($model['item_color'] ?? '', ENT_QUOTES, 'UTF-8'); ?>"
                                list="qc-color-options"
                                readonly
                              >
                              <span class="input-group-btn">
                                <button type="button" class="btn btn-default edit-color-btn">Edit</button>
                                <button type="button" class="btn btn-default cancel-color-btn hide">Cancel</button>
                              </span>
                            </div>
                          </td>
                          <td>
                          ...
                          <td>
                          <?php //echo (int)$product['item_cosmetic_passed'] === -1; ?>
                          ...
                          <td>
                            <select name="item_eu[]" class="item-eu form-control">
                              <option value="-1" >Select</option>
                                <option value="EU-UK" 
                                <?php echo $product['item_eu'] === "EU-UK" ? 'selected' :''?> >
                                  EU-UK</option>
                                <option value="US" 
                                <?php echo $product['item_eu'] === "US" ? 'selected' :''?> >
                                  US</option>
                            </select>
                          </td>
                          <td>
                    ...
                    <input type="submit" name="qc_save" value="Save" 
...
(function(){
// Globals (and initial/prev values)
let 
    ...
    itemEu = D['querySelectorAll']('.item-eu'),
...
// ==========
(function(){
  var colorCells = document.querySelectorAll('.qc-color-cell');
  colorCells.forEach(function(cell){
    var input = cell.querySelector('.qc-color-input');
    var editBtn = cell.querySelector('.edit-color-btn');
    var cancelBtn = cell.querySelector('.cancel-color-btn');
    if(!input || !editBtn || !cancelBtn){
      return;
    }
    ...
    cancelBtn.addEventListener('click', function(){
      if (typeof input.dataset.originalValue !== 'undefined') {
    ...
    input.addEventListener('keydown', function(event){
      if(event.key === 'Enter' && !input.hasAttribute('readonly')){
        ...
        editBtn.click();
      } else if(event.key === 'Escape' && !input.hasAttribute('readonly')){
        event.preventDefault();
        cancelBtn.click();
      }
    });
...
let 
  ...
  itemComments = D['querySelectorAll']('.item-comment'),
  itemEu = D['querySelectorAll']('.item-eu'),
  itemColorInputs = D['querySelectorAll']('.qc-color-input'),
  qcCompleted = D['querySelector']('.qc_completed');
  ...
  itemsCode.forEach((item,index)=>{
    prevItemsData.push({
      ...
      functionalPassed:+itemFunctionalPassed[index].value,
      flashed: +itemFlashed[index].value,
      eu: itemEu[index].value,
      comment: itemComments[index].value,
      color: itemColorInputs[index] ? itemColorInputs[index].value.trim() : "",
  ...
  let onSubmit = function(e){
    ...
    itemsCode.forEach((t, i)=>{
      ...
      if(itemColorInputs[i]){
        itemColorInputs[i].value = itemColorInputs[i].value.trim();
      }
    }) 
    ...
    let user_id = D['querySelector']('.user_id').value,
      ...
      ICP = [], // new item cosmetic passed
      IFP = [], // new item functional passed
      IFL = [], // new item flashed
    ...
    itemsCode.forEach((item,index)=>{
      ...
      const currentColor = itemColorInputs[index] ? itemColorInputs[index].value.trim() : "";
      if(
        ...
        itemEu[index].value != prevItemsData[index].eu ||
        itemComments[index].value != prevItemsData[index].comment ||
        currentColor != prevItemsData[index].color
      ){
        ...
        EU.push(itemEu[index].value);
        ICOLOR.push(currentColor);
