...
<?php 
  ...
  $return_id = $_GET['ret_id'];
  $purchase_details = mysqli_query($conn,"select 
  pr.return_id,
  pr.date,
  pr.item_code,
  pr.report_comment,
  ...
  from tbl_serial_purchase_return as pr 

  inner join tbl_serial_products as pu 
  on pr.item_code = pu.item_code 

  inner join tbl_serial_purchases as pur 
  on pur.purchase_id = pu.purchase_id

  where pr.return_id = '".$return_id."'")
    or die('error: '.mysqli_error($conn));
  $get_purchase_details = mysqli_fetch_assoc($purchase_details);
...
// $filename = $_POST["ExportType"] . ".xls";		 
  $filename = "PRID#".$get_purchase_details['return_id']."_".$get_supplier['name']."_".$get_purchase_details['date'].".xls";		 
            ...
            <div class="row">
              <div class="col-xs-12">
                ...
                invoiceHeader(
                  ...
                  "Return# : <span class='order_id'>SPR-". $return_id."</span>"
                );               
              ?>
                    <br>
                    ...
                    <div class="row">
                      ...
                      $query = mysqli_query($conn,"select 
                        ...
                        from tbl_serial_purchase_return as pr
                        inner join 
                        tbl_serial_products as po 
                        on po.item_code = pr.item_code 
                        where pr.return_id='".$return_id."'")
                        or die('error: '.mysqli_error($conn));                    
                     while($row = mysqli_fetch_assoc($query)): ?>
                        ...
                        <input type="text" value="<?php echo $return_id; ?>" class="comment-purchase-id hidden">
