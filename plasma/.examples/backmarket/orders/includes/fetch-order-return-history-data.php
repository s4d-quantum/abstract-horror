...
<?php
    ...
    $fetch_total_rows_count = mysqli_query($conn, "select count(distinct return_id) as total from tbl_order_return")
    ...
    if(isset($_POST['return_id'])){
        ...
        ret.order_id,
        tbl_orders.customer_id as customer,
        ret.date

        from tbl_order_return as ret

        inner join tbl_orders on 
        tbl_orders.order_id = ret.order_id 

        GROUP BY ret.return_id

        where ord.return_id='".$_POST['return_id']."'")
        or die('Error: '.mysqli_error($conn)); 
