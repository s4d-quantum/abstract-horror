...
function send_to_printer_webhook($zpl_content) {
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, PRINT_WEBHOOK_URL);
    curl_setopt($ch, CURLOPT_POST, true);
...
function find_uuid_and_label($cookies, $consignment, $parcel, $debug = false) {
    $headers = [
        'Cookie: ' . $cookies,
        'Accept: application/json'
    ];
    ...
    for ($page = $last_page; $page >= 1; $page--) {
        ...
        foreach ($shipments as $ship) {
            ...
            if (($outbound['consignmentNumber'] ?? '') === $consignment) {
                ...
                echo "📤 Sending to print webhook...\n";
                $webhook_response = send_to_printer_webhook($label_content);
...
try {
    ...
    find_uuid_and_label($cookies, $consignment, $parcel, $debug);
